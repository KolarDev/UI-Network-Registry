...
export default function TrackingLookup({ data, onEdit, onClose }: TrackingLookupProps) {
    ...
    const documents = [
        { title: 'Staff ID Card', url: data.staff_id_file_url },
        { title: 'Recent Payslip', url: data.payslip_file_url },
    ...
    return (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-4 sm:p-6 space-y-6">
                ...
                {/* Submitted documents (only when the storage disk exposes URLs) */}
                {documents.length > 0 && (
                    <div className="pt-2 border-t border-slate-100">
                        <h4 className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Submitted Documents</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {documents.map((doc) => (
                                <button
                                    key={doc.title}
                                    type="button"
                                    onClick={() => setPreview({ src: doc.url, title: doc.title })}
                                    className="w-full flex items-center justify-between gap-3 px-3 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-left transition-colors"
                                >
                                    ...
                                    <span className="text-xs font-bold text-[#2856C3] flex-shrink-0">View</span>
                                </button>
                            ))}
                        </div>
                    </div>
